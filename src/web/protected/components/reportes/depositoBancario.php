 <?php
    ...
    class depositoBancario extends Reportes 
    {
        public static function reporte($ids,$name,$type) 
        {
            ...
            $balance =  SaldoCabina::get_Model($ids);
            if($balance != NULL){
                ...
                foreach ($balance as $key => $registro) {
                    ...
                    $montoBanco = Deposito::model()->find("FechaCorrespondiente = '$registro->Fecha' AND CABINA_Id = $registro->CABINA_Id")->MontoBanco;
                    $montoDep = Deposito::model()->find("FechaCorrespondiente = '$registro->Fecha' AND CABINA_Id = $registro->CABINA_Id")->MontoDep;
                    $numRef = Deposito::model()->find("FechaCorrespondiente = '$registro->Fecha' AND CABINA_Id = $registro->CABINA_Id")->NumRef;
...
//                    
                    $table.=   '<tr >
                                    ...
                                    <td '.Reportes::defineStyleTd($key+2).'>'.Reportes::format(Reportes::defineMonto(10), $type).'</td>
