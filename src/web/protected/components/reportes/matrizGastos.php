...
class matrizGastos extends Reportes
{
    ...
    {
        /**
         * Porque esta linea esta comentada, de lo contrario eliminar
         */
                    ...
                    foreach($cabinas as $key => $cabina)
                    {
                        $sqlMontoGasto="SELECT SUM(d.Monto) AS Monto, d.status, d.moneda, 
                                               (SELECT d.Monto AS Monto
                                                ...
                                                WHERE d.CABINA_Id=c.id AND EXTRACT(YEAR FROM d.FechaMes) = '$año' AND EXTRACT(MONTH FROM d.FechaMes) = '$mes' AND d.TIPOGASTO_Id=t.id AND d.TIPOGASTO_Id=$gasto->TIPOGASTO_Id AND d.CABINA_Id = $cabina->Id AND d.status = 3 AND d.moneda = 1
                                                GROUP BY d.moneda) AS MontoDolares,
                                               (SELECT d.Monto AS Monto
                                                FROM detallegasto d, cabina c, tipogasto t
                                                WHERE d.CABINA_Id=c.id AND EXTRACT(YEAR FROM d.FechaMes) = '$año' AND EXTRACT(MONTH FROM d.FechaMes) = '$mes' AND d.TIPOGASTO_Id=t.id AND d.TIPOGASTO_Id=$gasto->TIPOGASTO_Id AND d.CABINA_Id = $cabina->Id AND d.status = 3 AND d.moneda = 2
                ...
                foreach($cabinas as $key => $cabina)
                {
                    $sqlTotales="SELECT (SELECT SUM(d.Monto) AS Monto 
                                        ...
                                        (SELECT SUM(d.Monto) AS Monto 
                                         FROM detallegasto AS d INNER JOIN tipogasto AS t ON d.TIPOGASTO_Id = t.id INNER JOIN cabina AS c ON d.CABINA_Id = c.id
                                         WHERE d.CABINA_Id = $cabina->Id AND EXTRACT(YEAR FROM d.FechaMes) = '$año' AND EXTRACT(MONTH FROM d.FechaMes) = '$mes' AND d.moneda = 2 AND d.status = 3) AS MontoS, 
                                        d.moneda
