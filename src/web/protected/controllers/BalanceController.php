    ...
    */
    public function actionCreateDeposito()
    {
        ...
        if (isset($_POST['Balance'])) {
            ...
            $sql = "SELECT id FROM balance where Fecha=:fecha AND CABINA_Id=:cabina";
    ...
    public function actionUpdateMonto()
    {
        ...
        while ($model->Id != null) {
            if (isset($_POST['MontoBanco' . $id]) && $_POST['MontoBanco' . $id] != null && is_numeric($_POST['MontoBanco' . $id])) {
                ...
                $model->DiferencialBancario = Yii::app()->format->formatDecimal(
                                                $_POST['MontoBanco' . $id]-($model->FijoLocal+$model->FijoProvincia
                                                +$model->FijoLima+$model->Rural+$model->Celular+$model->LDI+$model->RecargaCelularMov
                                                +$model->RecargaFonoYaMov+$model->RecargaCelularClaro+$model->RecargaFonoClaro+$model->OtrosServicios)
                                              );
                
                $model->ConciliacionBancaria = Yii::app()->format->formatDecimal(
                                                 $_POST['MontoBanco' . $id]-$model->MontoDeposito
                                               );
    ...
    public static function controlAcceso($tipoUsuario)
    {
        ...
        if($tipoUsuario == 2)
        {
            return array(
                ...
                array('label' => 'Reporte Libro Ventas', 'url' => array('reporteLibroVentas')),
                array('label' => 'Reporte Depositos Bancarios', 'url' => array('reporteDepositos')),
                array('label' => 'Reporte Brightstar', 'url' => array('reporteBrightstar')),
                ...
                array('label' => 'Tablero de Control de Actv.', 'url' => array('controlPanel')),
            );
        }
        /* ADMINISTRADOR */
        if($tipoUsuario == 3)
        {
            return array(
                array('label' => 'Tablero de Control de Actv.', 'url' => array('controlPanel')),
                array('label' => 'Administrar Balances', 'url' => array('admin')),
                array('label' => '__________REPORTES___________', 'url' => array('')),
                array('label' => 'Reporte Libro Ventas', 'url' => array('reporteLibroVentas')),
                array('label' => 'Reporte Depositos Bancarios', 'url' => array('reporteDepositos')),
                array('label' => 'Reporte Brightstar', 'url' => array('reporteBrightstar')),
