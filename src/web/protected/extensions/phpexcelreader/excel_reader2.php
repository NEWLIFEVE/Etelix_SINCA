...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
        ...
        function saveDB($tipo,$sheet=0,$table_class='excel') {
            
            Yii::import('webroot.protected.models.Balance');
            Yii::import('webroot.protected.models.Cabina');
            ...
            /*|*/      $columnaMontoMovistar = 8;                                                 /*|*/
            /*|*/      $columnaMontoClaro    = 8;                                                 /*|*/
            /*|*/                                                                                 /*|*/
            /*|*///******** COLUMNAS SELECCIONADAS EN LOS ARCHIVOS ETELIXPERU.XLS **************///*|*/
            /*|*/                                                                                 /*|*/
            /*|*/      $columnaFechaEtelixPeru      = 1;                                          /*|*/
            /*|*/      $columnaMontoEtelixPeru      = 5;                                          /*|*/
            /*|*/      $columnaCentinelaEtelixPeru1 = 3;                                          /*|*/
            /*|*/      $columnaCentinelaEtelixPeru2 = 4;                                          /*|*/
            /*|*/      $columnaCentinelaEtelixPeru3 = 6;                                          /*|*/
            /*|*/      $columnaCentinelaEtelixPeru4 = 7;                                          /*|*/
            /*|*/                                                                                 /*|*/
            /*|*///******** COLUMNAS SELECCIONADAS EN LOS ARCHIVOS CAPTURA.XLS *****************///*|*/
            /*|*/                                                                                 /*|*/
            /*|*/      $columnaFechaCaptura         = 1;                                          /*|*/
            /*|*/      $columnaNombreCabinaCaptura  = 2;                                          /*|*/
            /*|*/      $columnaMinutosCaptura       = 3;                                          /*|*/
            /*|*/      $columnaTraficoCapturaDollar = 4;                                          /*|*/
            /*|*/      //$columnaTraficoCapturaDollar = 3;                                          /*|*/
            /*|*/                                                                                 /*|*/
            /*|*///*****************************************************************************///*|*/
...
//                 $_SESSION['list'][0] = $a;
            ...
            elseif($tipo=="movistar"){
                ...
                for($row=7;$row<=$this->rowcount($sheet);$row++) {
                    ...
                    $fechaActual        = $this->val($row,$columnaFecha,$sheet);
                    $nombreCabinaActual = Yii::app()->format->formatearNombreCabina($this->val($row,$columnaNombreCabina,$sheet),"brightstar");
            ...
            elseif($tipo=="claro"){
                ...
                for($row=7;$row<=$this->rowcount($sheet);$row++) {
                    ...
                    $cabinaId = Cabina::model()->findBySql("SELECT Id FROM cabina where Nombre LIKE '%".$nombreCabinaActual."%';");

                    $registro = Balance::model()->findBySql("SELECT * FROM balance where Fecha='".$fechaActual."' AND CABINA_Id='".$cabinaId->Id."' AND RecargaVentasClaro is NULL;");
                    ...
                    if($registro!=NULL) {
                        ...
                        for($rowNext;$rowNext<=$this->rowcount($sheet);$rowNext++) {
                            ...
                            if($fechaActual == $this->val($rowNext,$columnaFecha,$sheet) 
                                ...
                                $montoAcum += Yii::app()->format->formatExcelDecimal($this->val($rowNext,$columnaMontoClaro,$sheet));
                                $iteracionMax++;
                        ...
                        $registro->RecargaVentasClaro = $montoAcum;
                        $registro->update();
                    }
                }
            }
            ...
            elseif($tipo=="captura"){
                ...
                for($row=4;$row<=$this->rowcount($sheet);$row++) {
                    ...
                    $fechaActual        = Yii::app()->format->formatearFecha($this->val($row,$columnaFechaCaptura,$sheet));
                    $nombreCabinaActual = Yii::app()->format->formatearNombreCabina($this->val($row,$columnaNombreCabinaCaptura,$sheet),"captura");

                    //=========== VALORES DE LAS COLUMNAS EN EN EXCEL - FIN ====================
                    
                    $cabinaId = Cabina::model()->findBySql("SELECT Id FROM cabina where Nombre LIKE '%".$nombreCabinaActual."%';");

                    $registro = Balance::model()->findBySql("SELECT * FROM balance where Fecha='".$fechaActual."' AND CABINA_Id='".$cabinaId->Id."' AND (TraficoCapturaDollar is NULL OR MinutosCaptura is NULL);");
                    ...
                    if($registro!=NULL) {
                        ...
                        for($rowNext;$rowNext<=$this->rowcount($sheet);$rowNext++) {
                            ...
                            if($fechaActual == Yii::app()->format->formatearFecha($this->val($rowNext,$columnaFechaCaptura,$sheet)) 
                                    && $nombreCabinaActual == Yii::app()->format->formatearNombreCabina($this->val($rowNext,$columnaNombreCabinaCaptura,$sheet),"captura")){

                                $montoAcum  += Yii::app()->format->formatExcelDecimal($this->val($rowNext,$columnaTraficoCapturaDollar,$sheet));
                                $minutoAcum += $this->val($rowNext,$columnaMinutosCaptura,$sheet);
                                $iteracionMax++;
                        ...
                        $registro->TraficoCapturaDollar = $montoAcum;
                        $registro->MinutosCaptura       = $minutoAcum;
                        $registro->update();
                    }
                }
            }
            
            elseif($tipo=="etelixPeru"){


                for($row=2;$row<=$this->rowcount($sheet);$row++) {

                    //if(stripos($this->val($row,6,$sheet), 'o') !== FALSE )                        continue;

                        //*********** VALORES DE LAS COLUMNAS EN EN EXCEL - COMIENZO ***************

                        $fechaActual = Yii::app()->format->formatearFecha($this->val($row,$columnaFechaEtelixPeru,$sheet),'etelixPeru');

                        //=========== VALORES DE LAS COLUMNAS EN EN EXCEL - FIN ====================

                        $registro = Balance::model()->findBySql("SELECT * FROM balance where Fecha='".$fechaActual."' AND CABINA_Id='17' AND RecargaVentasMov is NULL;");
                        ...
                        if($registro!=NULL) {
                            ...
                            for($rowNext;$rowNext<=$this->rowcount($sheet);$rowNext++) {
                                ...
                                if($fechaActual == Yii::app()->format->formatearFecha($this->val($rowNext,$columnaFechaEtelixPeru,$sheet),'etelixPeru')){

                                    $montoAcum += Yii::app()->format->formatExcelDecimal($this->val($rowNext,$columnaMontoEtelixPeru,$sheet));
